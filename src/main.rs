...
fn move_snake(world_config: World) -> i32 {
    let size = world_config.world_size * world_config.world_size;
    let head = world_config.snake.get_snake_head().unwrap();

    let col = head % world_config.world_size;
    let row = head / world_config.world_size;

    match world_config.snake.direction {
        snake::Direction::Right => ((row * world_config.world_size) + (col + 1)) % size,
        snake::Direction::Left => {
            if col == 0 {
                if row == 0 {
                    world_config.world_size * world_config.world_size
                } else {
                    (((row - 1) * world_config.world_size) + (world_config.world_size - 1)) % size
                }
            } else {
                ((row * world_config.world_size) + (col - 1)) % size
            }
        }
        snake::Direction::Up => {
            if row == 0 {
                if col == world_config.world_size - 1 {
                    ((world_config.world_size - 1) * world_config.world_size) % size
                } else {
                    (((world_config.world_size - 1) * world_config.world_size) + (col + 1)) % size
                }
            } else {
                (((row - 1) * world_config.world_size) + col) % size
            }
        }
        snake::Direction::Down => {
            if row == world_config.world_size - 1 {
                if col == 0 {
                    ((world_config.world_size * world_config.world_size)
                        + (world_config.world_size - 1))
                        % size
                } else {
                    (((world_config.world_size) * world_config.world_size) + (col - 1)) % size
                }
            } else {
                (((row + 1) * world_config.world_size) + col) % size
            }
